 ...
 * }
 *
 * urls Para realizar pruebas
http://localhost/Farmacias/GetFarmacias.php?Region=7&Comuna=BUIN
http://localhost/Farmacias/GetFarmacias.php?Region=7&Local=AHUMADA
http://localhost/Farmacias/GetFarmacias.php?Region=7&Comuna=BUIN&Local=CRUZ%20VERDE
    ...
    foreach ($user as $key => $value) {
        ...
        if(($Comuna==$value["comuna_nombre"] && $Local==$value["local_nombre"])||($Comuna==$value["comuna_nombre"] && $Local=="")||($Comuna=="" && $Local==$value["local_nombre"]))
        {
            if($firstComma==1)
                ...
                $firstComma=1;
                $XMLRes=$XMLRes.$Comma.'{comuna_nombre="'.$value["comuna_nombre"].'" Farmacia="'.$value["local_nombre"].'"  funcionamiento_hora_apertura="'.$value["funcionamiento_hora_apertura"].'" funcionamiento_hora_cierre="'. $value["funcionamiento_hora_cierre"].'" local_telefono="'. $value["local_telefono"].'" funcionamiento_dia="'.$value["funcionamiento_dia"].'" local_lat="'.$value["local_lat"].'" local_lng="'.$value["local_lng"].'"}';
